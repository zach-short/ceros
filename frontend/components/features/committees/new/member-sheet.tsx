...
export function MemberSheet({
  ...
  const [membersBuffer, setMembersBuffer] = useState<User[]>([
    ...selectedMembers,
  ]);
  ...
  const handleToggle = (member: User) => {
    ...
    if (isIncluded) {
      setMembersBuffer(
        membersBuffer.filter((listMember) => listMember.id !== member.id),
      );
    } else {
    ...
    <Sheet>
      ...
      <SheetContent>
        ...
        {selectedMembers.length === 0 && (
          <p className='px-4'>No member is selected</p>
        )}
        ...
        <SheetFooter>
          <SaveButton onSave={onSave} membersBuffer={membersBuffer} />
          <SheetClose asChild>
